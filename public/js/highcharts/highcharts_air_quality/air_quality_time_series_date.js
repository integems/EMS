...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch('http://localhost:3000/api/air_quality/date_time')
        ...
        .then(data => {
            ...
            // Prepare data series for each parameter with unique colors
            ...
            const seriesData = parameters.map((parameter, index) => ({
                ...
                data: airQualityData.map(entry => ({
                    x: new Date(entry.date_time).setHours(0, 0, 0, 0), // Set time to midnight
                    y: entry[parameter]
                })),
